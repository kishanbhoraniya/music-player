...
Future fetchSongDetails(songId) async {
  ...
  try {
    artist = (getMain[songId]['more_info']['artistMap']['primary_artists']).map((value)=> value['name']).join(', ');
  } catch (e) {
    artist = "";
  }
  try {
    albumArtist = (getMain[songId]['more_info']['artistMap']['artists']).map((value)=> value['name']).join(', ');
  } catch (e) {
    albumArtist = "";
  }
